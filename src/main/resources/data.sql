...
SELECT gen_random_uuid(), t.*, true, NOW(), NOW()
FROM (VALUES (
    'PURCHASE_EMAIL',
    ...
    CAST(ARRAY['nombre', 'evento', 'fecha', 'cantidad', 'total', 'codigo_qr'] AS TEXT ARRAY)
), (
    'REMINDER_EMAIL',
    ...
    CAST(ARRAY['nombre', 'evento', 'fecha', 'lugar', 'hora', 'codigo_qr'] AS TEXT ARRAY)
), (
    'CHANGE_EMAIL',
    'Cambio en evento',
    'EMAIL',
    ...
    CAST(ARRAY['nombre', 'evento', 'detalle_cambio', 'nueva_fecha', 'nuevo_lugar'] AS TEXT ARRAY)
), (
    'CANCELLATION_EMAIL',
    ...
    CAST(ARRAY['nombre', 'evento', 'fecha', 'motivo', 'total'] AS TEXT ARRAY)
), (
    'PROMOTION_EMAIL',
    ...
    CAST(ARRAY['nombre', 'evento', 'descuento', 'codigo_promocion', 'fecha_expiracion'] AS TEXT ARRAY)
)) AS t(code, name, channel, subject, body_template, variables)
WHERE NOT EXISTS (SELECT 1 FROM notification_templates nt WHERE nt.code = t.code);
