INSERT INTO notification_templates (id, code, name, channel, subject, body_template, variables, is_active, created_at, updated_at)
SELECT gen_random_uuid(), 'PURCHASE_EMAIL', 'Confirmación de compra', 'EMAIL', '¡Gracias por tu compra, {{nombre}}!',
...
ARRAY['nombre', 'evento', 'fecha', 'cantidad', 'total', 'codigo_qr'], true, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM notification_templates WHERE code = 'PURCHASE_EMAIL');

INSERT INTO notification_templates (id, code, name, channel, subject, body_template, variables, is_active, created_at, updated_at)
SELECT gen_random_uuid(), 'REMINDER_EMAIL', 'Recordatorio de evento', 'EMAIL', '📅 Recordatorio: {{evento}} es mañana',
'<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;padding:20px;max-width:600px;margin:auto;">
...
ARRAY['nombre', 'evento', 'fecha', 'lugar', 'hora', 'codigo_qr'], true, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM notification_templates WHERE code = 'REMINDER_EMAIL');

INSERT INTO notification_templates (id, code, name, channel, subject, body_template, variables, is_active, created_at, updated_at)
SELECT gen_random_uuid(), 'CHANGE_EMAIL', 'Cambio en evento', 'EMAIL', '🔔 Cambios en {{evento}}',
'<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;padding:20px;max-width:600px;margin:auto;">
...
ARRAY['nombre', 'evento', 'detalle_cambio', 'nueva_fecha', 'nuevo_lugar'], true, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM notification_templates WHERE code = 'CHANGE_EMAIL');

INSERT INTO notification_templates (id, code, name, channel, subject, body_template, variables, is_active, created_at, updated_at)
SELECT gen_random_uuid(), 'CANCELLATION_EMAIL', 'Cancelación de evento', 'EMAIL', '❌ Evento cancelado: {{evento}}',
'<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;padding:20px;max-width:600px;margin:auto;">
...
ARRAY['nombre', 'evento', 'fecha', 'motivo', 'total'], true, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM notification_templates WHERE code = 'CANCELLATION_EMAIL');

INSERT INTO notification_templates (id, code, name, channel, subject, body_template, variables, is_active, created_at, updated_at)
SELECT gen_random_uuid(), 'PROMOTION_EMAIL', 'Oferta especial para ti', 'EMAIL', '🎉 {{descuento}} off en {{evento}}',
'<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;padding:20px;max-width:600px;margin:auto;">
...
ARRAY['nombre', 'evento', 'descuento', 'codigo_promocion', 'fecha_expiracion'], true, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM notification_templates WHERE code = 'PROMOTION_EMAIL');
